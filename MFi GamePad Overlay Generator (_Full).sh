...
## _Full - File name definition
fileName_full=" (_Full)"

## Machine Specfic - File name definition
fileName_con=" (Console)"
fileName_arc=" (Arcade)"

## (Variant) Low - File name definition
fileName_Low="-lowbtn"

# _Full - Target ID atoms definition
...
# _Full - Target ID Name definition
tarId_Name_full[0]=""	# Defined Next

# _Full Redirection Table
...
# _Full - Target conunter definition
tarTot_full=0			# Denined Next


## _Full - Variabile fill-zone
...
echo "Generation of \"$fileName$fileName_Low$fileName_full.$fileExt\" in progress…"

echo "overlays = "$((tarTot_full+1))										| tee    "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null

for tarCount in `seq 0 $tarTot_full`; do
	echo ""															 		| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_full_screen = "$olayFS					 		| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_normalized = "$olayNorm				 		| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_name = \""${tarId_Name_full[$tarCount]}"\""	| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_range_mod = "$olayRangeMod				 		| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_alpha_mod = "$olayAlphaMod				 		| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
done
...
for tarCount in `seq 0 $tarTot_full`; do
	...
	case "${tarId_orie_full[$tarCount]}" in
		"${orie[0]}")
			...
			# Radial Shape																# Rectangular Shape
			     btnShow_[2]=${btnShow[2]}		      ;btnShow_[3]=${btnShow[3]}		;btnSelectCon_[2]=${btnSelectCon[2]} ;btnSelectCon_[3]=${btnSelectCon[3]}	
			     btnHide_[2]=${btnHide[2]}		      ;btnHide_[3]=${btnHide[3]}		 ;btnStartCon_[2]=${btnStartCon[2]}	  ;btnStartCon_[3]=${btnStartCon[3]}		
			   btnStaRew_[2]=${btnStaRew[2]}	    ;btnStaRew_[3]=${btnStaRew[3]}		    ;btnL3Low_[2]=${btnL3Low[2]}	     ;btnL3Low_[3]=${btnL3Low[3]}			
			   btnStaFfw_[2]=${btnStaFfw[2]}	    ;btnStaFfw_[3]=${btnStaFfw[3]}		    ;btnR3Low_[2]=${btnR3Low[2]}	     ;btnR3Low_[3]=${btnR3Low[3]}			
			   btnStaDec_[2]=${btnStaDec[2]}	    ;btnStaDec_[3]=${btnStaDec[3]}		     ;btnBtnM_[2]=${btnBtnM[2]}		      ;btnBtnM_[3]=${btnBtnM[3]}				
			...
			btnSelectArc_[2]=${btnSelectArc[2]}	 ;btnSelectArc_[3]=${btnSelectArc[3]}	
			 btnStartArc_[2]=${btnStartArc[2]}	  ;btnStartArc_[3]=${btnStartArc[3]}	
			       btnL3_[2]=${btnL3[2]}		        ;btnL3_[3]=${btnL3[3]}			
			       btnR3_[2]=${btnR3[2]}		        ;btnR3_[3]=${btnR3[3]}			
			 btnMach2Arc_[2]=${btnMach2Arc[2]}	  ;btnMach2Arc_[3]=${btnMach2Arc[3]}	
			 btnMach2Con_[2]=${btnMach2Con[2]}	  ;btnMach2Con_[3]=${btnMach2Con[3]}	
		...
		"${orie[1]}")
			...
			# Radial Shape																# Rectangular Shape
			     btnShow_[2]=${btnShow[3]}		      ;btnShow_[3]=${btnShow[2]}		;btnSelectCon_[2]=${btnSelectCon[5]} ;btnSelectCon_[3]=${btnSelectCon[6]}
			     btnHide_[2]=${btnHide[3]}		      ;btnHide_[3]=${btnHide[2]}		 ;btnStartCon_[2]=${btnStartCon[5]}	  ;btnStartCon_[3]=${btnStartCon[6]}
			   btnStaRew_[2]=${btnStaRew[3]}	    ;btnStaRew_[3]=${btnStaRew[2]}		    ;btnL3Low_[2]=${btnL3Low[5]}	     ;btnL3Low_[3]=${btnL3Low[6]}
			   btnStaFfw_[2]=${btnStaFfw[3]}	    ;btnStaFfw_[3]=${btnStaFfw[2]}		    ;btnR3Low_[2]=${btnR3Low[5]}	     ;btnR3Low_[3]=${btnR3Low[6]}
			   btnStaDec_[2]=${btnStaDec[3]}	    ;btnStaDec_[3]=${btnStaDec[2]}		     ;btnBtnM_[2]=${btnBtnM[5]}		      ;btnBtnM_[3]=${btnBtnM[6]}
			...
			btnSelectArc_[2]=${btnSelectArc[3]}	 ;btnSelectArc_[3]=${btnSelectArc[2]}	
			 btnStartArc_[2]=${btnStartArc[3]}	  ;btnStartArc_[3]=${btnStartArc[2]}	
			       btnL3_[2]=${btnL3[3]} 		        ;btnL3_[3]=${btnL3[2]}			
			       btnR3_[2]=${btnR3[3]} 		        ;btnR3_[3]=${btnR3[2]}			
			 btnMach2Arc_[2]=${btnMach2Arc[3]}	  ;btnMach2Arc_[3]=${btnMach2Arc[2]}	
			 btnMach2Con_[2]=${btnMach2Con[3]}	  ;btnMach2Con_[3]=${btnMach2Con[2]}	
	...
	echo ""																																							| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "# Overlay "$tarCount" - "${tarId_Name_full[$tarCount]}																									| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo ""																																							| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_descs = "$descTot																														| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
#	echo ${tarId_show_orie[$tarCount]}","${orie[0]}","${orie[1]}

	echo ""																																							| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc0 = \""${btnShow[0]}","${bgpTopX_[0]}","${bgpTopY_[0]}","${btnShow[1]}","${btnShow_[2]}","${btnShow_[3]}"\""						| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc0_overlay = "${btnShow[4]}																											| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc0_next_target = \""${tarTableShow_full[$tarCount]}"\""																				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc1 = \""${btnHide[0]}","${bgpTopX_[5]}","${bgpTopY_[5]}","${btnHide[1]}","${btnHide_[2]}","${btnHide_[3]}"\""						| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc1_overlay = "${btnHide[4]}																											| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc1_next_target = \""${tarTableHide_full[$tarCount]}"\""																				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo ""																																							| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc2 = \""${btnStaInc[0]}","${bgpSavX_[0]}","${bgpSavY_[0]}","${btnStaInc[1]}","${btnStaInc_[2]}","${btnStaInc_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc2_overlay = "${btnStaInc[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc3 = \""${btnStaDec[0]}","${bgpSavX_[1]}","${bgpSavY_[1]}","${btnStaDec[1]}","${btnStaDec_[2]}","${btnStaDec_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc3_overlay = "${btnStaDec[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo ""																																							| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc4 = \""${btnStaSave[0]}","${bgpSavX_[2]}","${bgpSavY_[2]}","${btnStaSave[1]}","${btnStaSave_[2]}","${btnStaSave_[3]}"\""			| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc4_overlay = "${btnStaSave[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc5 = \""${btnStaLoad[0]}","${bgpSavX_[3]}","${bgpSavY_[3]}","${btnStaLoad[1]}","${btnStaLoad_[2]}","${btnStaLoad_[3]}"\""			| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc5_overlay = "${btnStaLoad[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo ""																																							| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc6 = \""${btnStaRew[0]}","${bgpSavX_[4]}","${bgpSavY_[4]}","${btnStaRew[1]}","${btnStaRew_[2]}","${btnStaRew_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc6_overlay = "${btnStaRew[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc7 = \""${btnStaFfw[0]}","${bgpSavX_[5]}","${bgpSavY_[5]}","${btnStaFfw[1]}","${btnStaFfw_[2]}","${btnStaFfw_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc7_overlay = "${btnStaFfw[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo ""																																							| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc8 = \""${btnSelect_[0]}","${bgpBtmX_[0]}","${bgpBtmY_[0]}","${btnSelect_[1]}","${btnSelect_[2]}","${btnSelect_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc8_overlay = "${btnSelect_[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc9 = \""${btnStart_[0]}","${bgpBtmX_[1]}","${bgpBtmY_[1]}","${btnStart_[1]}","${btnStart_[2]}","${btnStart_[3]}"\""					| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	echo "overlay"$tarCount"_desc9_overlay = "${btnStart_[4]}																										| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
	...
	case "${tarId_show_full[$tarCount]}" in
		"${show[1]}"|"${show[2]}")
			echo ""																																					| tee -a "$fileName$fileName_Low$fileName_full.$fileExt"								  		> /dev/null
			echo "overlay"$tarCount"_desc10 = \""${btnL3Low__[0]}","${bgpTmLX_[0]}","${bgpTmLY_[0]}","${btnL3Low__[1]}","${btnL3Low__[2]}","${btnL3Low__[3]}"\""	| tee -a "$fileName$fileName_Low$fileName_full.$fileExt"								  		> /dev/null
			echo "overlay"$tarCount"_desc10_overlay = "${btnL3Low__[4]}																								| tee -a "$fileName$fileName_Low$fileName_full.$fileExt"								  		> /dev/null
			echo "overlay"$tarCount"_desc11 = \""${btnR3Low__[0]}","${bgpTmLX_[1]}","${bgpTmLY_[1]}","${btnR3Low__[1]}","${btnR3Low__[2]}","${btnR3Low__[3]}"\""	| tee -a "$fileName$fileName_Low$fileName_full.$fileExt"								  		> /dev/null
			echo "overlay"$tarCount"_desc11_overlay = "${btnR3Low__[4]}																								| tee -a "$fileName$fileName_Low$fileName_full.$fileExt"								  		> /dev/null
			;;
	esac
	...
	case "${tarId_show_full[$tarCount]}" in
		"${show[2]}")
			echo ""																																					| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc12 = \""${btnBtnM[0]}","${bgpTopX_[1]}","${bgpTopY_[1]}","${btnBtnM[1]}","${btnBtnM_[2]}","${btnBtnM_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc12_overlay = "${btnBtnM[4]}																								| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc12_next_target = \""${tarTableChgBtnM_full[$tarCount]}"\""																	| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc13 = \""${btnMach_[0]}","${bgpTopX_[2]}","${bgpTopY_[2]}","${btnMach_[1]}","${btnMach_[2]}","${btnMach_[3]}"\""			| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc13_overlay = "${btnMach_[4]}																								| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc13_next_target = \""${tarTableChgMach_full[$tarCount]}"\""																	| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc14 = \""${btnMenu[0]}","${bgpTopX_[3]}","${bgpTopY_[3]}","${btnMenu[1]}","${btnMenu_[2]}","${btnMenu_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc14_overlay = "${btnMenu[4]}																								| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc15 = \""${btnOrie[0]}","${bgpTopX_[4]}","${bgpTopY_[4]}","${btnOrie[1]}","${btnOrie_[2]}","${btnOrie_[3]}"\""				| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc15_overlay = "${btnOrie[4]}																								| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			echo "overlay"$tarCount"_desc15_next_target = \""${tarTableChgOrie_full[$tarCount]}"\""																	| tee -a "$fileName$fileName_full.$fileExt" "$fileName$fileName_Low$fileName_full.$fileExt"	> /dev/null
			;;
	esac
