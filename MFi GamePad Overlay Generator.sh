...
btnBtnM[0]="overlay_next"			;btnBtnM[1]="rect"			;btnBtnM[2]="0.036"			;btnBtnM[3]="0.030"			;btnBtnM[4]="start_genesis.png"			;btnBtnM[5]="0.064"			;btnBtnM[6]="0.0169"
...
bgpTopX_L[4]="0.63"	 ;bgpTopY_L[4]="0.09"	# Orientation Switch Button
bgpTopX_L[5]="0.80"	 ;bgpTopY_L[5]="0.09"	# Shader Next
bgpTopX_L[6]="0.95"	 ;bgpTopY_L[6]="0.09"	# Hide Button
...
bgpBtmX_L[2]="0.5"	 ;bgpBtmY_L[2]="0.90"	# Reset Button
...
bgpTopX_P[4]="0.645" ;bgpTopY_P[4]="0.09"	# Orientation Switch Button
bgpTopX_P[5]="0.777" ;bgpTopY_P[5]="0.05"	# Shader Next
bgpTopX_P[6]="0.91"	 ;bgpTopY_P[6]="0.05"	# Hide Button

# Saves Manage Group(Portrait)
bgpSavX_P[0]="0.09"	 ;bgpSavY_P[0]="0.57"	# Save State Button
bgpSavX_P[1]="0.91"	 ;bgpSavY_P[1]="0.57"	# Load State Button
bgpSavX_P[2]="0.09"	 ;bgpSavY_P[2]="0.67"	# State Slot Dec Button
bgpSavX_P[3]="0.91"	 ;bgpSavY_P[3]="0.67"	# State Slot Inc Button
...
for tarCount in `seq 0 $tarTot`; do
	case $((tarCount % 4)) in
		0|1|2)
			...
			;;
		3)
			tarTableShow[$tarCount]=${tarId_Name[$((tarCount-3))]}
			;;
	esac
done
...
for tarCount in `seq 0 $tarTot`; do
	case $((tarCount % 4)) in
		0)
			tarTableHide[$tarCount]=${tarId_Name[$((tarCount+3))]}
			;;
		1|2|3)
		...
		for tarCount in `seq 0 $tarTot`; do			
			...
			case "${tarId_orie[$tarCount]}" in
				"${orie[0]}")
					...
					bgpTopX_[6]=${bgpTopX_L[6]} ;bgpTopY_[6]=${bgpTopY_L[6]}	
					...
					# Radial Shape																# Rectangular Shape
					   ...
					   btnStaShow_[2]=${btnStaShow[2]}	   ;btnStaShow_[3]=${btnStaShow[3]}		
				...
				"${orie[1]}")
					...
					bgpTopX_[6]=${bgpTopX_P[6]} ;bgpTopY_[6]=${bgpTopY_P[6]}																															
					...
					# Radial Shape																# Rectangular Shape
					   ...
					   btnStaShow_[2]=${btnStaShow[3]}	   ;btnStaShow_[3]=${btnStaShow[2]}		
			...
			case "${tarId_savM[$tarCount]}" in
				...
				"${savM[1]}")
					...
					offsetThumb=$descAddsavM
					  offsetTop=$descAddsavM
					;;
			esac
			...
			echo ""																																														>> "$outName"
			echo "overlay"$tarCount"_desc0 = \""${btnShow[0]}","${bgpTopX_[0]}","${bgpTopY_[0]}","${btnShow[1]}","${btnShow_[2]}","${btnShow_[3]}"\""													>> "$outName"
			echo "overlay"$tarCount"_desc0_overlay = "${btnShow[4]}																																		>> "$outName"
			echo "overlay"$tarCount"_desc0_next_target = \""${tarTableShow[$tarCount]}"\""																												>> "$outName"
			echo "overlay"$tarCount"_desc1 = \""${btnHide[0]}","${bgpTopX_[6]}","${bgpTopY_[6]}","${btnHide[1]}","${btnHide_[2]}","${btnHide_[3]}"\""													>> "$outName"
			echo "overlay"$tarCount"_desc1_overlay = "${btnHide[4]}																																		>> "$outName"
			echo "overlay"$tarCount"_desc1_next_target = \""${tarTableHide[$tarCount]}"\""																												>> "$outName"

			## Visualization Level - Variable Buttons
			case "${tarId_show[$tarCount]}" in
				# Visualization Level - NOT minimal (normal, partial, full) (Add to previous)
				"${show[0]}"|"${show[1]}"|"${show[2]}")			

					# Select / Star Buttons
					echo ""																																												>> "$outName"
					echo "overlay"$tarCount"_desc2 = \""${btnSelect_[0]}","${bgpBtmX_[1]}","${bgpBtmY_[1]}","${btnSelect_[1]}","${btnSelect_[2]}","${btnSelect_[3]}"\""							>> "$outName"
					echo "overlay"$tarCount"_desc2_overlay = "${btnSelect_[4]}																															>> "$outName"
					echo "overlay"$tarCount"_desc3 = \""${btnStart_[0]}","${bgpBtmX_[3]}","${bgpBtmY_[3]}","${btnStart_[1]}","${btnStart_[2]}","${btnStart_[3]}"\""								>> "$outName"
					echo "overlay"$tarCount"_desc3_overlay = "${btnStart_[4]}																															>> "$outName"
										
					# Save State Visualization controls					
					echo ""																																												>> "$outName"
					echo "overlay"$tarCount"_desc4 = \""${btnStaCtrl_[0]}","${bgpBtmX_[0]}","${bgpBtmY_[0]}","${btnStaCtrl_[1]}","${btnStaCtrl_[2]}","${btnStaCtrl_[3]}"\""								>> "$outName"
					echo "overlay"$tarCount"_desc4_overlay = "${btnStaCtrl_[4]}																															>> "$outName"
					echo "overlay"$tarCount"_desc4_next_target = \""${tarTableChgSavM[$tarCount]}"\""																									>> "$outName"
					echo "overlay"$tarCount"_desc5 = \""${btnStaCtrl_[0]}","${bgpBtmX_[4]}","${bgpBtmY_[4]}","${btnStaCtrl_[1]}","${btnStaCtrl_[2]}","${btnStaCtrl_[3]}"\""								>> "$outName"
					echo "overlay"$tarCount"_desc5_overlay = "${btnStaCtrl_[4]}																															>> "$outName"
					echo "overlay"$tarCount"_desc5_next_target = \""${tarTableChgSavM[$tarCount]}"\""																									>> "$outName"
					...
					case "${tarId_savM[$tarCount]}" in
						"${savM[1]}")
							# State selection
							echo ""																																										>> "$outName"
							echo "overlay"$tarCount"_desc6 = \""${btnStaInc[0]}","${bgpSavX_[0]}","${bgpSavY_[0]}","${btnStaInc[1]}","${btnStaInc_[2]}","${btnStaInc_[3]}"\""							>> "$outName"
							echo "overlay"$tarCount"_desc6_overlay = "${btnStaInc[4]}																													>> "$outName"
							echo "overlay"$tarCount"_desc7 = \""${btnStaDec[0]}","${bgpSavX_[1]}","${bgpSavY_[1]}","${btnStaDec[1]}","${btnStaDec_[2]}","${btnStaDec_[3]}"\""							>> "$outName"
							echo "overlay"$tarCount"_desc7_overlay = "${btnStaDec[4]}																													>> "$outName"
							# State operations
							echo ""																																										>> "$outName"
							echo "overlay"$tarCount"_desc8 = \""${btnStaSave[0]}","${bgpSavX_[2]}","${bgpSavY_[2]}","${btnStaSave[1]}","${btnStaSave_[2]}","${btnStaSave_[3]}"\""						>> "$outName"
							echo "overlay"$tarCount"_desc8_overlay = "${btnStaSave[4]}																													>> "$outName"
							echo "overlay"$tarCount"_desc9 = \""${btnStaLoad[0]}","${bgpSavX_[3]}","${bgpSavY_[3]}","${btnStaLoad[1]}","${btnStaLoad_[2]}","${btnStaLoad_[3]}"\""						>> "$outName"
							echo "overlay"$tarCount"_desc9_overlay = "${btnStaLoad[4]}																													>> "$outName"
							# Specials controls (Rewind, Fast Forward)
							echo ""																																										>> "$outName"
							echo "overlay"$tarCount"_desc10 = \""${btnStaRew[0]}","${bgpSavX_[4]}","${bgpSavY_[4]}","${btnStaRew[1]}","${btnStaRew_[2]}","${btnStaRew_[3]}"\""							>> "$outName"
							echo "overlay"$tarCount"_desc10_overlay = "${btnStaRew[4]}																													>> "$outName"
							echo "overlay"$tarCount"_desc11 = \""${btnStaFfw[0]}","${bgpSavX_[5]}","${bgpSavY_[5]}","${btnStaFfw[1]}","${btnStaFfw_[2]}","${btnStaFfw_[3]}"\""							>> "$outName"
							echo "overlay"$tarCount"_desc11_overlay = "${btnStaFfw[4]}																													>> "$outName"
							;;
					esac
					...
					case "${tarId_show[$tarCount]}" in
						"${show[1]}"|"${show[2]}")
							echo ""																																										>> "$outName"
							echo "overlay"$tarCount"_desc"$((6+$offsetThumb))" = \""${btnL3___[0]}","${bgpTmbX__[0]}","${bgpTmbY__[0]}","${btnL3___[1]}","${btnL3___[2]}","${btnL3___[3]}"\""			>> "$outName"
							echo "overlay"$tarCount"_desc"$((6+$offsetThumb))"_overlay = "${btnL3___[4]}																								>> "$outName"
							echo "overlay"$tarCount"_desc"$((7+$offsetThumb))" = \""${btnR3___[0]}","${bgpTmbX__[1]}","${bgpTmbY__[1]}","${btnR3___[1]}","${btnR3___[2]}","${btnR3___[3]}"\""			>> "$outName"
							echo "overlay"$tarCount"_desc"$((7+$offsetThumb))"_overlay = "${btnR3___[4]}																								>> "$outName"
							;;
					esac
					...
					case "${tarId_show[$tarCount]}" in
						"${show[2]}")
							echo ""																																										>> "$outName"
							# Top Bar
							echo "overlay"$tarCount"_desc"$((8+$offsetTop))" = \""${btnShaPre[0]}","${bgpTopX_[1]}","${bgpTopY_[1]}","${btnShaPre[1]}","${btnShaPre_[2]}","${btnShaPre_[3]}"\""			>> "$outName"
							echo "overlay"$tarCount"_desc"$((8+$offsetTop))"_overlay = "${btnShaPre[4]}																									>> "$outName"
							echo "overlay"$tarCount"_desc"$((9+$offsetTop))" = \""${btnBtnM[0]}","${bgpTopX_[2]}","${bgpTopY_[2]}","${btnBtnM[1]}","${btnBtnM_[2]}","${btnBtnM_[3]}"\""					>> "$outName"
							echo "overlay"$tarCount"_desc"$((9+$offsetTop))"_overlay = "${btnBtnM[4]}																									>> "$outName"
							echo "overlay"$tarCount"_desc"$((9+$offsetTop))"_next_target = \""${tarTableChgBtnM[$tarCount]}"\""																			>> "$outName"
							echo "overlay"$tarCount"_desc"$((10+$offsetTop))" = \""${btnMenu[0]}","${bgpTopX_[3]}","${bgpTopY_[3]}","${btnMenu[1]}","${btnMenu_[2]}","${btnMenu_[3]}"\""				>> "$outName"
							echo "overlay"$tarCount"_desc"$((10+$offsetTop))"_overlay = "${btnMenu[4]}																									>> "$outName"
							echo "overlay"$tarCount"_desc"$((11+$offsetTop))" = \""${btnOrie[0]}","${bgpTopX_[4]}","${bgpTopY_[4]}","${btnOrie[1]}","${btnOrie_[2]}","${btnOrie_[3]}"\""				>> "$outName"
							echo "overlay"$tarCount"_desc"$((11+$offsetTop))"_overlay = "${btnOrie[4]}																									>> "$outName"
							echo "overlay"$tarCount"_desc"$((11+$offsetTop))"_next_target = \""${tarTableChgOrie[$tarCount]}"\""																		>> "$outName"
							echo "overlay"$tarCount"_desc"$((12+$offsetTop))" = \""${btnShaNex[0]}","${bgpTopX_[5]}","${bgpTopY_[5]}","${btnShaNex[1]}","${btnShaNex_[2]}","${btnShaNex_[3]}"\""		>> "$outName"
							echo "overlay"$tarCount"_desc"$((12+$offsetTop))"_overlay = "${btnShaNex[4]}																								>> "$outName"
							echo "overlay"$tarCount"_desc"$((13+$offsetTop))" = \""${btnReset[0]}","${bgpBtmX_[2]}","${bgpBtmY_[2]}","${btnReset[1]}","${btnReset_[2]}","${btnReset_[3]}"\""			>> "$outName"
							echo "overlay"$tarCount"_desc"$((13+$offsetTop))"_overlay = "${btnReset[4]}																									>> "$outName"
							;;
					esac
			esac
